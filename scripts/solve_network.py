...
def add_CCL_constraints(
    ...
    if config["solving"]["agg_p_nom_limits"]["agg_heat_pumps"]:
        rename_heat_pumps = {
            "urban central air heat pump": "heat_pump-all",
            "rural air heat pump": "heat_pump-all",
            "rural ground heat pump": "heat_pump-all",
            "urban decentral air heat pump": "heat_pump-all",
        }
        links = links.replace(rename_heat_pumps)
    grouper = pd.concat([gens.bus.map(n.buses.country), gens.carrier], axis=1)
    grouper_links = pd.concat([links.bus0.map(n.buses.country), links.carrier], axis=1)
    ...
    if config["solving"]["agg_p_nom_limits"]["include_existing"]:
        ...
        if config["solving"]["agg_p_nom_limits"]["agg_heat_pumps"]:
            links_cst = links_cst.replace(rename_heat_pumps)
        rhs_cst = (
            pd.concat(
        ...
        rhs_cst_links = (
            pd.concat(
                [
                    links_cst.bus0.map(n.buses.country),
        ...
        rhs_cst_links.index = rhs_cst_links.index.rename({"bus0": "country"})
