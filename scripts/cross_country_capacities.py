...
if __name__ == "__main__":
    ...
    if "country" not in network.buses.columns:
        network.buses["country"] = network.buses.index.map(
    ...
    csv_df["Country Pair"] = csv_df.apply(
        lambda row: "-".join(sorted([row["From"], row["To"]])), axis=1
    )

    aggregated_csv = (
        csv_df.groupby("Country Pair")
        .agg({"NTC_F": "mean", "NTC_B": "mean"})
        .reset_index()
    )
    aggregated_csv["Ember NTC (MW)"] = (
        aggregated_csv["NTC_F"] + aggregated_csv["NTC_B"]
    ) / 2

    # Merge and compare
    comparison_df = pd.merge(
        capacities_df,
        aggregated_csv[["Country Pair", "Ember NTC (MW)"]],
        on="Country Pair",
        how="outer",
    ).fillna(0)
    comparison_df["Difference (MW)"] = (
        comparison_df["Model Capacity (MW)"] - comparison_df["Ember NTC (MW)"]
    )

    # Filter for selected countries
    focus_comparison = comparison_df[
        comparison_df["Country Pair"].str.contains("DE|NL|IT|PL|CZ|GR", na=False)
    ]
    ...
    focus_comparison.plot(
        x="Country Pair",
        y=["Model Capacity (MW)", "Ember NTC (MW)"],
        kind="bar",
        figsize=(14, 8),
        width=0.35,
    )
    plt.title(
        "Comparison of Model vs Ember NTC Interconnection Capacities (2024) for Focus Countries"
    )
