...
network_path = (
    ...
    else f"results/validation_{year}/networks/base_s_39_elec_.nc"
)
ember_capacity_data_path = (
    snakemake.input.ember_capacity
    if "snakemake" in globals()
    else "validation/ember_data/yearly_full_release_long_format.csv"
)
ember_demand_data_path = (
    snakemake.input.ember_demand
    if "snakemake" in globals()
...
regions_onshore_path = (
    ...
    if "snakemake" in globals()
    else f"resources/validation_{year}/country_shapes.geojson"
)
...
def process_ember_capacity():
    ...
    ember_capacity_filtered = ember_capacity_filtered[
        ~ember_capacity_filtered["Area"].isin(exclude_areas)
    ]
    ...
    ember_capacity_filtered["ISO"] = ember_capacity_filtered["ISO 3 code"].apply(
        iso3_to_iso2
    )
    ember_capacity_filtered = ember_capacity_filtered[
        ["ISO", "Variable", "Value", "Unit"]
    ]
    ember_capacity_pivot = ember_capacity_filtered.pivot_table(
        index="ISO", columns="Variable", values="Value"
    ).fillna(0)
    print(
        f"Processed Ember capacity data sample:\n{ember_capacity_pivot.head().to_string()}"
    )
...
def process_ember_demand():
    ...
    ember_demand_europe = ember_demand_europe[
        ember_demand_europe["Date"].str.startswith(str(year))
    ]
    ember_demand_europe = ember_demand_europe[ember_demand_europe["Unit"] == "TWh"]
    ember_demand_europe = ember_demand_europe[
        ember_demand_europe["Subcategory"] == "Demand"
    ]
    ember_demand_europe = ember_demand_europe[
        ["ISO", "Date", "Variable", "Value", "Unit"]
    ]

    ember_demand_yearly = ember_demand_europe.groupby(
        ["ISO", "Variable"], as_index=False
    )["Value"].sum()
    ember_demand_yearly["Unit"] = "TWh"
    ember_demand_yearly = ember_demand_yearly.set_index("ISO").drop(
        columns=["Unit", "Variable"]
    )
    print(
        f"Processed Ember demand data sample:\n{ember_demand_yearly.head().to_string()}"
    )
...
def process_pypsa_capacity():
    ...
    sto_country_tech = (
        n.storage_units.groupby(["bus", "carrier"])["p_nom"]
        .sum()
        .unstack(fill_value=0)
        .reset_index()
    )
    ...
    link_country_tech = (
        links_eff.groupby(["bus1", "carrier"])["effective_p_nom"]
        .sum()
        .unstack(fill_value=0)
        .reset_index()
    ).rename(columns={"bus1": "bus"})

    pypsa_country_tech = pd.concat(
        [pypsa_country_tech, sto_country_tech, link_country_tech], ignore_index=True
    )
    pypsa_country_tech["country"] = pypsa_country_tech["bus"].str[:2]

    pypsa_country_tech_merged = (
        pypsa_country_tech.groupby("country").sum(numeric_only=True).reset_index()
    )

    wind_cols = [
        col
        for col in pypsa_country_tech_merged.columns
        if col.startswith("onwind") or col.startswith("offwind")
    ]
    solar_cols = [
        col for col in pypsa_country_tech_merged.columns if col.startswith("solar")
    ]

    pypsa_country_tech_merged = merge_and_replace(
        pypsa_country_tech_merged, "Wind", wind_cols
    )
    pypsa_country_tech_merged = merge_and_replace(
        pypsa_country_tech_merged, "Solar", solar_cols
    )
    pypsa_country_tech_merged = merge_and_replace(
        pypsa_country_tech_merged, "Gas", ["CCGT", "OCGT", "urban central gas CHP"]
    )
    pypsa_country_tech_merged = merge_and_replace(
        pypsa_country_tech_merged, "Coal", ["coal", "lignite"]
    )
    pypsa_country_tech_merged = merge_and_replace(
        pypsa_country_tech_merged, "Hydro", ["hydro", "ror", "PHS"]
    )
    pypsa_country_tech_merged = merge_and_replace(
        pypsa_country_tech_merged, "Other Fossil", ["oil"]
    )
    pypsa_country_tech_merged = merge_and_replace(
        pypsa_country_tech_merged, "Nuclear", ["nuclear"]
    )
    ...
    pypsa_country_tech_merged = merge_and_replace(
        pypsa_country_tech_merged, "Bioenergy", ["urban central solid biomass CHP"]
    )
    pypsa_country_tech_merged = (
        pypsa_country_tech_merged.set_index("country").div(1000).round(2)
    )
    print(
        f"Processed PyPSA capacity data sample:\n{pypsa_country_tech_merged.head().to_string()}"
    )
...
def process_pypsa_demand():
    ...
    print(
        f"Processed PyPSA demand data sample:\n{pypsa_loads_yearly.head().to_string()}"
    )
...
def plot_network():
    ...
    fig, ax = plt.subplots(
        figsize=(12, 12), subplot_kw={"projection": ccrs.EqualEarth(n.buses.x.mean())}
    )
    ...
    output_path = (
        snakemake.output.network_plot
        if "snakemake" in globals()
        else f"results/validation_{year}/plots/network_plot.png"
    )
...
def plot_total_capacity_comparison(ember_capacity, pypsa_capacity):
    ...
    ax.set_title(
        "Comparison of Installed Capacity by Technology (Summed Across Europe)"
    )
    ax.legend()
    plt.tight_layout()

    output_path = (
        snakemake.output.total_capacity_plot
        if "snakemake" in globals()
        else f"results/validation_{year}/plots/total_capacity_plot.png"
    )
...
def plot_capacity_comparison_horizontal(countries, ember_capacity, pypsa_capacity):
    ...
    for idx, country in enumerate(countries[:6]):
        ax = axes[idx]
        ember_row = (
            ember_capacity.loc[country] if country in ember_capacity.index else None
        )
        pypsa_row = (
            pypsa_capacity.loc[country] if country in pypsa_capacity.index else None
        )
        ...
        if ember_row is not None and pypsa_row is not None:
            ...
            ax.barh(
                y + height / 2, pypsa_vals, height, label="PyPSA-Eur", color="#192238"
            )
            ax.set_yticks(y)
    ...
    output_path = (
        snakemake.output.country_capacity_plot
        if "snakemake" in globals()
        else f"results/validation_{year}/plots/country_capacity_plot.png"
    )
...
def plot_demand_comparison(ember_demand, pypsa_demand):
    ...
    common_countries = pypsa_loads_yearly_flat.index.intersection(
        ember_demand_yearly_flat.index
    )
    pypsa_vals = pypsa_loads_yearly_flat.loc[common_countries]
    ember_vals = ember_demand_yearly_flat.loc[common_countries]

    df_compare = pd.DataFrame(
        {"PyPSA-Eur": pypsa_vals, "Ember": ember_vals}, index=common_countries
    )
    ...
    for ax, countries, title in zip(
        ...
        ax.barh(
            y - width / 2,
            df_compare.loc[countries, "PyPSA-Eur"],
            height=width,
            label="PyPSA-Eur",
            color="#192238",
        )
        ax.barh(
            y + width / 2,
            df_compare.loc[countries, "Ember"],
            height=width,
            label="Ember",
            color="#13ce74",
        )
    ...
    output_path = (
        snakemake.output.demand_plot
        if "snakemake" in globals()
        else f"results/validation_{year}/plots/demand_plot.png"
    )
