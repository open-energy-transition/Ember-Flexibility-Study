...
def apply_2023_nuclear_decommissioning(n, year=2023):
    if year == 2023:
        nuclear_info = {
            "Isar 2": {
                "coords": [12.29315, 48.60560556],
                "country": "DE",
                "dateout": "2023-04-15",
            },
            "Emsland": {
                "coords": [7.317858333, 52.47423056],
                "country": "DE",
                "dateout": "2023-04-15",
            },
            "Neckarwestheim 2": {
                "coords": [9.175, 49.04111111],
                "country": "DE",
                "dateout": "2023-04-15",
            },
        }
    ...
    for plant, info in nuclear_info.items():
        ...
        network_coords = n.buses.loc[
            n.links.query("carrier == 'nuclear' and bus1 in @country_buses").bus1,
            ["x", "y"],
        ].set_index(
            n.links.query("carrier == 'nuclear' and bus1 in @country_buses").index
        )
        ...
        # decommission
        n.links_t.p_max_pu[nearest_gen] = n.links.loc[nearest_gen].p_max_pu * (
            (n.snapshots < dateout).astype(int)
        )
...
def apply_hourly_fuel_prices(n, carriers, fn_hourly_prices):
    ...
    if not df.index.equals(n.snapshots):
        logger.warning(
            "Snapshot indices do not match exactly. Overwriting prices index with network snapshots."
        )
...
def apply_custom_pf_constraint(
    n,
    ...
    if link_name not in n.links.index:
        ...
        raise KeyError(
            f"Link '{link_name}' not found in n.links.index. Examples: {some}"
        )
    ...
    if len(idx_arr) == 0:
        few = list(labels[:5])
        raise KeyError(
            f"Link '{link_name}' not found in variable coord '{dim}'. "
    ...
    if bidirectional:
        p_pos = m.add_variables(
            coords={"snapshot": n.snapshots}, name=f"{link_name}_AtoB_pos", lb=0
        )
