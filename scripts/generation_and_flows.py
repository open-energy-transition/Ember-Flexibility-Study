...
network_path = (
    ...
    else "results/validation_2023/networks/base_s_39_elec_.nc"
)
ember_monthly_data_path = (
    snakemake.input.ember_monthly
    if "snakemake" in globals()
    else "validation/ember_data/europe_monthly_full_release_long_format.csv"
)
ember_yearly_data_path = (
    snakemake.input.ember_yearly
    if "snakemake" in globals()
    else "validation/ember_data/yearly_full_release_long_format.csv"
)
power_flows_data_path = (
    snakemake.input.power_flows
    if "snakemake" in globals()
    else "validation/entsoe_data/physical_energy_power_flows_2023.csv"
)
...
def process_ember_generation_yearly():
    ...
    if not os.path.exists(ember_yearly_data_path):
        print(
            f"Yearly CSV not found at {ember_yearly_data_path}. Falling back to monthly aggregation."
        )
        return None
    ...
    try:
        ...
        print(
            f"Detected columns: ISO={iso_col}, Variable={variable_col}, Value={value_col}, "
...
def process_pypsa_generation():
    ...
    gen_energy = (
        n.generators_t.p.T.multiply(n.snapshot_weightings.generators).T.sum(axis=0)
        / 1e6
    )  # MWh to TWh
    ...
    gen_grouped = gen_energy.groupby(["country", "carrier"], as_index=False)[
        "Value"
    ].sum()
    gen_grouped["Ember_Variable"] = (
        gen_grouped["carrier"].map(pypsa_to_ember).fillna(gen_grouped["carrier"])
    )
    gen_renamed = gen_grouped.groupby(["country", "Ember_Variable"], as_index=False)[
        "Value"
    ].sum()
    gen_renamed = gen_renamed.rename(
        columns={"country": "ISO", "Ember_Variable": "Variable"}
    ).round()

    sto_meta = n.storage_units[["bus", "carrier"]].copy()
    sto_meta.loc[:, "country"] = sto_meta["bus"].str[:2]
    sto_energy = (
        n.storage_units_t.p.T.multiply(n.snapshot_weightings.stores).T.sum(axis=0) / 1e6
    )  # MWh to TWh
    ...
    sto_grouped = sto_energy.groupby(["country", "carrier"], as_index=False)[
        "Value"
    ].sum()
    sto_grouped["Ember_Variable"] = (
        sto_grouped["carrier"].map(pypsa_to_ember).fillna(sto_grouped["carrier"])
    )
    sto_renamed = sto_grouped.groupby(["country", "Ember_Variable"], as_index=False)[
        "Value"
    ].sum()
    sto_renamed = sto_renamed.rename(
        columns={"country": "ISO", "Ember_Variable": "Variable"}
    ).round()

    gen_and_sto = pd.concat([gen_renamed, sto_renamed], ignore_index=True)
    gen_and_sto = (
        gen_and_sto.groupby(["ISO", "Variable"], as_index=False)["Value"].sum().round()
    )
...
def process_pypsa_generation_sector():
    ...
    vres_carriers = [
        ...
        "ror",
        "solar rooftop",
    ]  # ideally this is not hardcoded !
    vres = n.generators.query("carrier in @vres_carriers").index
    gen_energy = (
        n.generators_t.p.T.multiply(n.snapshot_weightings.generators)
        .loc[vres]
        .T.sum(axis=0)
        / 1e6
    )  # MWh to TWh
    ...
    conv_buses = list(
        n.generators.query(
            "carrier in ['gas', 'coal', 'uranium', 'lignite', 'biomass', 'oil', 'solid biomass', 'unsustainable solid biomass']"
        ).bus
    )
    ...
    gen_energy_links = (
        -n.links_t.p1[gen_links]
        .T.multiply(n.snapshot_weightings.generators)
        .T.sum(axis=0)
        / 1e6
    )
    ...
    gen_energy_links = gen_energy_links.merge(
        link_meta, left_on="links", right_index=True
    )
    gen_grouped_links = gen_energy_links.groupby(
        ["country", "carrier"], as_index=False
    )["Value"].sum()
    gen_grouped_links["Ember_Variable"] = (
        gen_grouped_links["carrier"].map(pypsa_to_ember).fillna(gen_grouped["carrier"])
    )
    gen_renamed_links = gen_grouped_links.groupby(
        ["country", "Ember_Variable"], as_index=False
    )["Value"].sum()
    gen_renamed_links = gen_renamed_links.rename(
        columns={"country": "ISO", "Ember_Variable": "Variable"}
    ).round()
    ...
    gen_and_sto = pd.concat(
        [gen_renamed, gen_renamed_links, sto_renamed], ignore_index=True
    )
    gen_and_sto = (
        gen_and_sto.groupby(["ISO", "Variable"], as_index=False)["Value"].sum().round()
    )
...
def compare_ember_processing(df_yearly, df_monthly, country_iso="DE"):
    ...
    ax.bar(
        x - bar_width / 2, yearly_values, bar_width, label="Yearly CSV", color="skyblue"
    )
    ax.bar(
        x + bar_width / 2,
        monthly_values,
        bar_width,
        label="Monthly Aggregated",
        color="salmon",
    )
    ...
    output_path = (
        snakemake.output.ember_comparison
        if "snakemake" in globals()
        else f"results/validation_{year}/plots/ember_comparison_de.png"
    )
...
# Plotting functions
def plot_country_generation_mix_donut_subplots(
    ember_generation_yearly, country_isos, color_dict=None
):
    n = len(country_isos)
    ...
    for idx, country_iso in enumerate(country_isos):
        ...
        colors = (
            [color_dict.get(tech, "#cccccc") for tech in data.index]
            if color_dict
            else plt.cm.Set2.colors[: len(data)]
        )
        ...
        ax.text(
            0, 0, country_iso, ha="center", va="center", fontsize=18, fontweight="bold"
        )
    ...
    fig.suptitle(
        f"Yearly Electricity Generation by Technology\n(TWh, Ember {year})",
        fontsize=16,
        weight="bold",
        y=1.12,
    )
    ...
    output_path = (
        snakemake.output.donut_subplots
        if "snakemake" in globals()
        else f"results/validation_{year}/plots/donut_subplots.png"
    )
    os.makedirs(os.path.dirname(output_path), exist_ok=True)
    plt.savefig(output_path, bbox_inches="tight", dpi=300)
    plt.close()
...
def plot_country_generation_mix_donut_comparison(
    df1, df2, country_isos, color_dict=None, df1_label="Ember", df2_label="PyPSA"
):
    n = len(country_isos)
    ...
    for idx, country_iso in enumerate(country_isos):
        for j, (pivot_df, label) in enumerate(
            zip([pivot1, pivot2], [df1_label, df2_label])
        ):
            ax = axes[idx, j]
            if country_iso not in pivot_df.index:
                ax.axis("off")
            ...
            centre_circle = plt.Circle(
                (0, 0), 0.25, color="white", fc="white", linewidth=0
            )
            ax.add_artist(centre_circle)
            ...
            ax.set_title(
                f"{label}\n{country_iso}" if idx == 0 else country_iso,
                fontsize=14,
                fontweight="bold",
            )
            if idx == 0 and j == 0:
    ...
    fig.suptitle(
        "Yearly Electricity Generation [TWh]", fontsize=16, weight="bold", y=1.05
    )
    plt.tight_layout(rect=[0, 0, 1, 0.98])

    output_path = (
        snakemake.output.donut_comparison
        if "snakemake" in globals()
        else f"results/validation_{year}/plots/donut_comparison.png"
    )
    os.makedirs(os.path.dirname(output_path), exist_ok=True)
    plt.savefig(output_path, bbox_inches="tight", dpi=300)
    plt.close()
...
def analyze_power_flows():
    ...
    for sep in [";", ","]:
        ...
        raise ValueError(
            "Unable to parse CSV file with ';' or ',' as separator. Check file format."
        )
    ...
    avg_flows_2023 = (
        flows_2023_focus.groupby([from_area_col, to_area_col, "Month"])["Value"]
    ...
    power_flows_focus = (
        n.lines_t.p0[focus_lines]
        .reset_index()
        .melt(id_vars="snapshot", var_name="line", value_name="p0")
    )
    ...
    monthly_flows_pypsa = (
        power_flows_focus.groupby(["Connection", "Month"])["abs_p0"]
        .mean()
        .reset_index(name="Avg_Abs_Flow_PyPSA_MW")
    )
    ...
    if comparison_df.empty:
        print(
            "Warning: No common connections found between ENTSO-E 2023 and PyPSA-Eur data"
        )
        return []
    ...
    for connection in valid_connections:
        ...
        output_path = (
            f"results/validation_{year}/plots/power_flow_{connection_safe}.png"
        )
    ...
    output_list_path = (
        snakemake.output.power_flows_list
        if "snakemake" in globals()
...
if __name__ == "__main__":
    ...
    compare_ember_processing(
        ember_generation_yearly_alt, ember_generation_yearly, country_iso="DE"
    )
