...
if __name__ == "__main__":
    ...
    total_demand_per_carrier_MWh = (
        network.loads_t.p_set.groupby(carrier_map, axis=1).sum().sum(axis=0)
    )
    ...
    public_df = pd.DataFrame(
        {"Carrier": public_data.keys(), "Public Data (TWh)": public_data.values()}
    )
    ...
    plt.bar(
        ...
        color=colors[1],
        alpha=0.6,
        label="Public Data Total",
        edgecolor="black",
    )

    # Customize title and labels
    plt.title(
        "Comparison of Demand by Carrier (TWh)", fontsize=16, weight="bold", pad=15
    )
    ...
    # Customize x-axis
    plt.xticks(
        [p for p in x] + [len(comparison_df) - 2, len(comparison_df) - 1],
    ...
    print("\nTotal demand per carrier (TWh):")
    for carrier, pypsa_demand, public_demand in zip(
        comparison_df["Carrier"][:-2],
        comparison_df["PyPSA-Eur (TWh)"][:-2],
        comparison_df["Public Data (TWh)"][:-2],
    ):
        print(
            f"- {carrier}: PyPSA-Eur = {pypsa_demand:.2f} TWh, Public Data = {public_demand:.2f} TWh"
        )
