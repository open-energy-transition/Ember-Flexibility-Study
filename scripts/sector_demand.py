...
def get_carrier(col):
    """Extract the carrier name from the column name."""
    parts = col.split()
...
if __name__ == "__main__":
    ...
    carrier_map = {col: get_carrier(col) for col in network.loads_t.p_set.columns}
    ...
    total_demand_per_carrier_MWh = network.loads_t.p_set.groupby(carrier_map, axis=1).sum().sum(axis=0)

    # Convert to TWh
    total_demand_per_carrier_TWh = total_demand_per_carrier_MWh / 1.0e6
    ...
    plt.bar([p - width / 2 for p in x], comparison_df['PyPSA-Eur (TWh)'][:-2], width, label='PyPSA-Eur',
            color=colors[0], edgecolor='black')
    plt.bar([p + width / 2 for p in x], comparison_df['Public Data (TWh)'][:-2], width, label='Public Data',
            color=colors[1], edgecolor='black')

    # Plot total bars
    plt.bar(len(comparison_df) - 2, total_pypsa, width, color=colors[0], alpha=0.6, label='PyPSA-Eur Total',
            edgecolor='black')
    plt.bar(len(comparison_df) - 1, total_public, width, color=colors[1], alpha=0.6, label='Public Data Total',
            edgecolor='black')
    ...
    for i, (pypsa_val, public_val) in enumerate(
            zip(comparison_df['PyPSA-Eur (TWh)'][:-2], comparison_df['Public Data (TWh)'][:-2])):
        plt.text(i - width / 2, pypsa_val + 10, f"{pypsa_val:.2f}", ha='center', va='bottom', fontsize=9, color='black')
        plt.text(i + width / 2, public_val + 10, f"{public_val:.2f}", ha='center', va='bottom', fontsize=9,
                 color='black')

    plt.text(len(comparison_df) - 2, total_pypsa + 10, f"{total_pypsa:.2f}", ha='center', va='bottom', fontsize=9,
             color='black')
    plt.text(len(comparison_df) - 1, total_public + 10, f"{total_public:.2f}", ha='center', va='bottom', fontsize=9,
             color='black')
    ...
    print("\nTotal demand per carrier (TWh):")
    for carrier, pypsa_demand, public_demand in zip(comparison_df['Carrier'][:-2],
                                                    comparison_df['PyPSA-Eur (TWh)'][:-2],
                                                    comparison_df['Public Data (TWh)'][:-2]):
