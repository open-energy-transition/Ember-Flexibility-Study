...
if not config["ntc"]["enable"]:
    # If disabled, just copy the input to output
    import shutil
    shutil.copy(snakemake.input.network, snakemake.output.network)
    print("NTC disabled; network copied unchanged.")
    exit()

# Load NTC CSV
df = pd.read_csv(snakemake.input.ntc_file)

# Get year from wildcard (e.g., 2025)
year = int(config["scenario"]["planning_horizons"][0])
...
print(f"NTCs applied for year {year} (interpolated if needed).")
